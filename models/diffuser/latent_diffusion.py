...
def apply_conditioning(x, cond, scale):
...
class LatentDiffusion(nn.Module):
    def __init__(self, model, horizon, latent_dim, n_timesteps=1000, scale_input=True,
        ...
        self.scale = math.sqrt(latent_dim // 2) if scale_input else 1.
    ...
    def p_sample_loop(self, shape, cond, returns=None, verbose=True, return_diffusion=False):
        ...
        x = apply_conditioning(x, cond, self.scale)
    ...
    def p_losses(self, x_start, cond, t, returns=None):
        ...
        x_noisy = apply_conditioning(x_noisy, cond, self.scale)
        ...
        if self.predict_epsilon:
            ...
            x_recon = apply_conditioning(x_recon, cond, self.scale)
